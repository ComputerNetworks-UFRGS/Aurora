...
from manager.models.virtual_interface import (
...
def index(request):
    ...
    view_vars = {
        ...
        'actions': [{
            ...
            'url': "/Aurora/manager/virtual_machines/new/"
        }, {
            'name': "Synchronize",
            'url': "/Aurora/manager/virtual_machines/sync/"
        }]
    }
...
def detail(request, virtual_machine_id):
    ...
    view_vars = {
        ...
        'actions': [{
            'name': "Back to List",
            'url': "/Aurora/manager/virtual_machines/"
        }, {
            'name': "XML",
            'url': "/Aurora/manager/virtual_machines/" +
...
def start(request, virtual_machine_id):
    ...
    try:
        ...
        session_flash.set_flash(request,
            "Could not start Virtual Machine %s!" % vm.name, "error"
        )
...
def console(request, virtual_machine_id):
    ...
    try:
        ...
        session_flash.set_flash(request, "Could load console information for %s: %s" % (vm.name, str(e)), "error")
...
def shutdown(request, virtual_machine_id):
    ...
    try:
        ...
        session_flash.set_flash(request, "Could not shutdown Virtual Machine %s: %s" % (vm.name, str(e)), "error")
...
def resume(request, virtual_machine_id):
    ...
    try:
        ...
        session_flash.set_flash(request, "Could not resume Virtual Machine %s: %s" % (vm.name, str(e)), "error")
...
def suspend(request, virtual_machine_id):
    ...
    try:
        ...
        session_flash.set_flash(request, "Could not suspend Virtual Machine %s: %s" % (vm.name, str(e)), "error")
...
def sync(request):
    try:
        ...
        session_flash.set_flash(request, "Problems loading hosts", "error")
        raise redirect('manager-virtual-machines-index')
    ...
    for h in hosts:
        ...
        try:
            ...
            session_flash.set_flash(request, 'Problems synchronizing host "' + str(h) + '": ' + str(e), "error")
            return redirect('manager-virtual-machines-index')
...
def xml(request, virtual_machine_id):
    ...
    if xml_desc == False:
        session_flash.set_flash(request, "Could not read XML description for Virtual Machine %s" % vm.name, "error")
        return redirect(request.META['HTTP_REFERER'])
    else:
...
class VirtualMachineForm(forms.Form):
    action = "/Aurora/manager/virtual_machines/new/"
    ...
    if request.method == 'POST': # If the form has been submitted...
        ...
        if form.is_valid(): # All validation rules pass
            ...
            try:
                ...
                session_flash.set_flash(request, "Problems deploying Virtual Machine: %s" % str(e), "error")

            return redirect('manager-virtual-machines-index') # Redirect after POST
    else:
...
class VirtualMachineMigrationForm(forms.Form):
    action = "/Aurora/manager/virtual_machines/%s/migrate/"
    ...
    if request.method == 'POST': # If the form has been submitted...
        ...
        if form.is_valid(): # All validation rules pass
            ...
            try:
                ...
                session_flash.set_flash(request, "Could not migrate Virtual Machine %s: %s" % (vm.name, str(e)), "error")
...
class VirtualInterfaceForm(forms.Form):
    action = "/Aurora/manager/virtual_machines/%s/new_virtual_interface/"
...
def new_virtual_interface(request, virtual_machine_id):
    ...
    if request.method == 'POST': # If the form has been submitted...
        ...
        if form.is_valid(): # All validation rules pass
            ...
            return redirect('/Aurora/manager/virtual_machines/' + virtual_machine_id + '/') # Redirect after POST
    else:
    ...
    view_vars = {
        ...
        'actions': [
            { 'name': "Back to Details", 'url': "/Aurora/manager/virtual_machines/" + virtual_machine_id + "/" },
        ]
    }
    # insert slice_id in the action url
    form.action = form.action % virtual_machine_id
