...
from manager.models.event import Event, EVENT_STATES, RELATIONAL_OPERATION
from manager.models.metric import Metric
...
def index(request):
    ...
    view_vars = {
        ...
        'title': "Events List",
        'actions': [{ 'name': "New Event", 'url': "/Aurora/manager/events/new/" }]
    }
    c = Context({
...
def detail(request, event_id):
    view_vars = {
        ...
        'title': "Event Details",
        'actions': [
            { 'name': "Back to List", 'url': "/Aurora/manager/events/" },
        ]
    }
    try:
...
class EventForm(forms.Form):
    action = "/Aurora/manager/events/new/"
...
def new(request):
    if request.method == 'POST': # If the form has been submitted...
        ...
        if form.is_valid(): # All validation rules pass
            ...
            return redirect('manager-events-index') # Redirect after POST
    else:
...
def delete(request, event_id):
    ...
    return redirect('manager-events-index')
