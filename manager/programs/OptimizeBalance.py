...
class OptimizeBalance(OptimizationProgram):
    ...
    def optimize(self):
        logger.debug("OptimizeBalance program invoked")
    ...
    def pick_highest_residual_capacity_host(self, host_list, vm):
        # Initial highest residual capacity is the capacity of the origin host + requested_mem
        requested_mem = vm.memory/1024
        capacity = vm.host.get_info()
        mem_allocation = vm.host.get_memory_allocation()
        highest_residual_capacity = capacity['memory'] - (mem_allocation['total'] / 1024) + requested_mem
        # Init new candidate 
        new_candidate = None
        for candidate in host_list:
            ...
            #logger.debug("Candidate %s: total %d - %d - %d = %d | Highest: %d (%s)" % (candidate, capacity['memory'], (mem_allocation['total'] / 1024), requested_mem, free_mem_capacity, highest_residual_capacity, str(free_mem_capacity > highest_residual_capacity and free_mem_capacity >= requested_mem)))
