...
class FatTree(Topo):
    ...
    def __init__(self, num_pods=4, host_multiplier=1):
        Topo.__init__(self)
        ...
        for i in range(num_pods):
            ...
            for j in range(num_pods/2):
                ...
                # (multiplier can be used to increase number of hosts double/triple)
                for k in range((num_pods/2)*host_multiplier):
                    hindex += 1
                    pods[current_pod]['hosts'].append(self.addHost('h' + str(hindex)))
                    # Link the host just created to the edge switch at hand
                    self.addLink(
                        pods[current_pod]['edge_switches'][j],
                        pods[current_pod]['hosts'][-1] # host just appended
                    )
